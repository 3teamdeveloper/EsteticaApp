...
export default function EmployeesPage() {
  ...
  const confirmDeleteEmployee = async () => {
    ...
    try {
      ...
      toast.push('success', t('messages.deleted_success'));
    } catch (err) {
      setError(err instanceof Error ? err.message : t('errors.delete'));
      toast.push('error', t('errors.delete'));
    } finally {
  ...
  const handleInvite = async (id: number) => {
    try {
      ...
      if (!res.ok) throw new Error(data.error || t('errors.invite'));
      if (data.emailSendWarning) {
        toast.push('info', data.emailSendWarning);
      } else {
        toast.push('success', t('messages.invite_sent'));
      }
    } catch (err) {
      toast.push('error', err instanceof Error ? err.message : t('errors.invite'));
    }
  };
  ...
  return (
    <div className="p-1 md:p-6 mt-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          onClick={() => router.push(`/${locale}/dashboard/employees/new`)}
          disabled={isTrialInactive}
          className={isTrialInactive ? 'opacity-50 cursor-not-allowed' : ''}
          title={isTrialInactive ? t('upgrade_to_add') : ''}
        >
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {employee.lastInvitationSentAt && (
                              <span className="text-[11px] text-gray-500">
                                {t('status.sent_at')} {new Date(employee.lastInvitationSentAt).toLocaleString()}
                              </span>
                            )}
                          </div>
                        ) : (
                          employee.email && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{t('status.no_account')}</span>
                          )
                        )}
                      </div>
                      {employee.email && (
                  ...
                  <div className="flex flex-col gap-2 mt-4 sm:mt-0 sm:flex-row sm:items-center">
                    ...
                    <div className="flex w-full justify-end gap-2 flex-nowrap overflow-x-auto sm:w-auto sm:justify-start">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => router.push(`/${locale}/dashboard/employees/${employee.id}/schedules`)}
                        title={t('actions.manage_schedules')}
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => router.push(`/${locale}/dashboard/employees/${employee.id}`)}
                        disabled={isTrialInactive}
                        title={isTrialInactive ? t('upgrade_to_edit') : t('actions.edit')}
                      >
                      ...
                      {employee.email && (
                        <Button
                          ...
                          title={employee.hasPassword ? t('actions.account_active') : t('actions.invite')}
      ...
      {deleteModal.open && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg max-w-sm w-full">
            ...
            <p className="mb-6">{t('delete_modal.message')} "{deleteModal.employee?.name}"?</p>
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setDeleteModal({ open: false })}>
