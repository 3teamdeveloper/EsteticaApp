...
export default function Dashboard() {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <div className="text-red-600 text-lg mb-2">⚠️ {t('error_title')}</div>
  ...
  // Removed getStatusText - now using translations directly
  ...
  return (
    <div className="space-y-6 md:space-y-8 p-4 md:p-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">
              {t('welcome')}, {userData?.name || tc('user')}! 
            </h1>
      ...
      <div>
        <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-4 md:mb-6">{t('business_summary')}</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          <StatCard
            title={t('stats.total_services')}
      ...
      {todayAppointments.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/${locale}/dashboard/management`} className="text-sm text-rose-600 hover:text-rose-700 font-medium">
              {t('view_all')} →
            </Link>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusBadge(apt.status)}`}>
                      {t(`stats.${apt.status.toLowerCase()}`)}
                    </span>
                  </div>
          ...
          {todayAppointments.length > 5 && (
            <p className="text-xs text-gray-500 text-center mt-3">
              +{todayAppointments.length - 5} {t('more_appointments')}
            </p>
          )}
        </div>
      )}
      ...
      {/* Acciones rápidas */}
      <div>
        <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-4 md:mb-6">{t('quick_actions')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {/* Mostrar solo si NO es EMPLEADO */}
          {userData?.role !== 'EMPLEADO' && (
            <QuickActionCard
              title={t('actions.manage_services')}
              description={t('actions.manage_services_desc')}
              icon={
          ...
          {userData?.role !== 'EMPLEADO' && (
            <QuickActionCard
              title={t('actions.public_profile')}
              description={t('actions.public_profile_desc')}
              icon={
                <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
                </svg>
          ...
          {userData?.role !== 'EMPLEADO' && (
            <QuickActionCard
              title={t('actions.manage_employees')}
              description={t('actions.manage_employees_desc')}
              icon={
                <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3z" />
                </svg>
          ...
          <QuickActionCard
            title={t('actions.view_stats')}
            description={t('actions.view_stats_desc')}
            icon={
              <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
          ...
          <QuickActionCard
            ...
            description={t('actions.notifications_desc')}
            icon={
              <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
          ...
          <QuickActionCard
            ...
            description={t('actions.settings_desc')}
            icon={
              <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
