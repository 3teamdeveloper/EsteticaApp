...
export default function BusinessHoursConfig() {
  const t = useTranslations('settings.business_hours');
  ...
  const days = [
    { key: 0, nameKey: "sunday" },
    { key: 1, nameKey: "monday" },
    { key: 2, nameKey: "tuesday" },
    { key: 3, nameKey: "wednesday" },
    { key: 4, nameKey: "thursday" },
    { key: 5, nameKey: "friday" },
    { key: 6, nameKey: "saturday" },
  ];
  ...
  const saveBusinessHours = async () => {
    ...
    try {
      ...
      toast.push('error', t('messages.error'));
    } finally {
      setSaving(false);
    }
  };
            ...
            return (
              <div key={day.key} className="border border-gray-400 rounded-lg p-4 bg-gray-50">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-medium text-gray-900">{t(`days.${day.nameKey}`)}</h3>
                  <Button
                    variant="outline"
                    size="sm"
                ...
                {hasSchedules ? (
                  ...
                  <p className="text-gray-500 text-sm italic">{t('no_schedules')}</p>
                )}
              </div>
            );
          })}
