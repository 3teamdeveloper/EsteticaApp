...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.acceptTerms) {
      setError(t('errors.accept_terms'));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || t('errors.register_error'));
      }

      toast.push('success', t('success'));
      router.push(`/${locale}/login`);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : t('errors.register_error');
        ...
        <div className="w-full max-w-md">
          <div className="bg-white rounded-lg shadow-md border border-gray-200 p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label htmlFor="phone" className="block text-sm font-medium text-gray-700">
                  {t('phone')} <span className="text-gray-400 text-xs">{t('phone_optional')}</span>
                </label>
                <input
                  id="phone"
                  name="phone"
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  disabled={loading}
                >
                  <option value="salon">{t('business_types.salon')}</option>
                  <option value="barbershop">{t('business_types.barbershop')}</option>
                  <option value="spa">{t('business_types.spa')}</option>
                  <option value="nails">{t('business_types.nails')}</option>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">{t('password_help')}</p>
              </div>
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="acceptTerms" className="font-medium text-gray-700">
                      {t('accept_terms')}{" "}
                      <Link href={`/${locale}/terms`} className="text-rose-600 hover:text-rose-500">
                        {t('terms')}
                      </Link>{" "}
                      {t('and')}{" "}
                      <Link href={`/${locale}/privacy`} className="text-rose-600 hover:text-rose-500">
                        {t('privacy')}
                      </Link>
                    </label>
                  </div>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-white text-gray-500">{t('or_register_with')}</span>
                </div>
              </div>
            ...
            <div className="mt-8 space-y-4">
              <h3 className="text-sm font-medium text-gray-700">{t('benefits_title')}</h3>
              <ul className="space-y-2">
                <li className="flex items-start">
                  <CheckCircle className="h-5 w-5 text-rose-600 mr-2 flex-shrink-0" />
                  <span className="text-sm text-gray-600">{t('benefits.trial')}</span>
                </li>
                <li className="flex items-start">
                  <CheckCircle className="h-5 w-5 text-rose-600 mr-2 flex-shrink-0" />
                  <span className="text-sm text-gray-600">{t('benefits.features')}</span>
                </li>
                <li className="flex items-start">
                  <CheckCircle className="h-5 w-5 text-rose-600 mr-2 flex-shrink-0" />
                  <span className="text-sm text-gray-600">{t('benefits.support')}</span>
                </li>
              </ul>
            </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-600">
                {t('have_account')}{" "}
                <Link href={`/${locale}/login`} className="font-medium text-rose-600 hover:text-rose-500">
                  {t('login_link')}
                </Link>
              </p>
            </div>
